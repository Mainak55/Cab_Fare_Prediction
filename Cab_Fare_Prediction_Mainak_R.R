...
setwd("C:/Users/Mainak Sarkar/Desktop/Edwisor/Project/Cab_Fare_Prediciton")
...
{
  k_value[m,1] = m
  predictions = knnregTrain(training_set[,1:5], test_set[,1:5], training_set$fare_amount, k = m)
  K_val = regr.eval(exp(test_set[, 6]), exp(as.numeric(as.character(predictions))) , stats = "rmse")
  k_value[m,2] = K_val
}
#renaming the column
names(k_value)[1] = "K"
names(k_value)[2] = "RMSE"
plot(k_value$K,k_value$RMSE, type = "l", col = "red", xlab = "K", ylab = "RMSE")

#So from the graph we can see that the elbow is at k = 2, but here we want lower rmse values for our model
#So we will choose k=6 as a fair value

#Predicting the results with K = 6
predictions_KNN = knnregTrain(training_set[,1:5], test_set[,1:5], training_set$fare_amount, k = 6)

#Calculating different Error Metrics
KNN = regr.eval(exp(test_set[, 6]), exp(as.numeric(as.character(predictions_KNN))) , stats = c("mape", "rmse"))


#To compare all models
Error_metrics = as.data.frame(cbind(LR,DT,RF,SVR,KNN))
#Changing row names
