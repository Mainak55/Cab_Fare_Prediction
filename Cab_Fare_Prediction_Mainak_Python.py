# -*- coding: utf-8 -*-
"""
Created on Wed May 22 16:54:05 2019
...
import os
os.chdir("C:/Users/Mainak Sarkar/Desktop/Edwisor/Project/Cab_Fare_Prediciton")
...
for l in range(1,10):
    l = l+1
    model = KNeighborsRegressor(n_neighbors = l).fit(training_set.iloc[:, 0:5], training_set.iloc[:, 5])
    predicitons = model.predict(test_set.iloc[:, 0:5])
    from sklearn.metrics import mean_squared_error
    from math import sqrt
    error = sqrt(mean_squared_error(np.exp(test_set.iloc[:, 5]), np.exp((predicitons).astype(float))))
    k_value.append(error)

#Elbow Curve
elbow = pd.DataFrame(k_value)
elbow.plot()

#So from the graph we can see that the elbow is at k = 6, but here we want lower rmse values for our model
#So we will choose k=6 as a fair value

#Building the model with k = 6
KNN_model = KNeighborsRegressor(n_neighbors = 6).fit(training_set.iloc[:, 0:5], training_set.iloc[:, 5])
predictions_KNN = KNN_model.predict(test_set.iloc[:, 0:5])

#Calculating different Error Metrics
KNN = pd.DataFrame(MAPE_RMSE(np.exp(test_set.iloc[:, 5]), np.exp((predictions_KNN).astype(float))))



#To compare all models
Error_Metrics = pd.concat([LR, DT, RF, SVR, KNN], axis = 1)
Error_Metrics.columns = [ 'LR', 'DT', 'RF', 'SVR', 'KNN']
